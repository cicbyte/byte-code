...
func (s *sProject) GetProject(ctx context.Context, id int) (res *api.ProjectDetailRes, err error) {
	...
	err = g.DB().Model("projects p").Ctx(ctx).
		...
		LeftJoin("project_members po", "po.project_id = p.id AND po.role = 'owner'").
		LeftJoin("sys_users ou", "ou.id = po.user_id").
		Fields(`p.id, p.code, p.name, p.description, p.created_by,
